...
from gr6j import CemaNeige, ForcingData, Parameters, run
from gr6j.inputs import Catchment
from gr6j.model.run import _run_numba, _step_numba, step
from gr6j.model.types import State
...
class TestRunNumbaEquivalence:
    ...
    def test_full_run_equivalence(self, params: Parameters, forcing: ForcingData) -> None:
        ...
        np.testing.assert_allclose(result.gr6j.streamflow, streamflow_py, rtol=1e-10)
...
class TestNumericalStability:
    ...
    def test_long_simulation_stability(self) -> None:
        ...
        assert np.all(np.isfinite(result.gr6j.streamflow))
        assert np.all(np.isfinite(result.gr6j.production_store))
        assert np.all(np.isfinite(result.gr6j.routing_store))
        assert np.all(np.isfinite(result.gr6j.exponential_store))
    ...
    def test_extreme_parameter_combinations(self) -> None:
        ...
        for params in param_sets:
            result = run(params, forcing)
            assert np.all(np.isfinite(result.gr6j.streamflow)), f"Non-finite output for {params}"
            assert np.all(result.gr6j.streamflow >= 0), f"Negative streamflow for {params}"


class TestWithSnowCoupling:
    """Tests for Numba GR6J when coupled with CemaNeige snow module."""
    ...
    def test_snow_coupled_run_valid(
        self, snow_params: Parameters, catchment: Catchment, forcing_with_temp: ForcingData
    ) -> None:
        """Snow-coupled run produces valid outputs."""
        result = run(snow_params, forcing_with_temp, catchment)

        assert result.snow is not None
        assert len(result.gr6j.streamflow) == len(forcing_with_temp)
        assert np.all(result.gr6j.streamflow >= 0)
        assert np.all(np.isfinite(result.gr6j.streamflow))
    ...
    def test_snow_precip_modification(
        self, snow_params: Parameters, catchment: Catchment, forcing_with_temp: ForcingData
    ) -> None:
        """Snow module correctly modifies precipitation to GR6J."""
        result = run(snow_params, forcing_with_temp, catchment)

        # precip to GR6J should differ from raw precip on cold days
        # (snow accumulates instead of passing through)
        assert not np.allclose(result.gr6j.precip, forcing_with_temp.precip)
